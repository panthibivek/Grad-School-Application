...
def parser_error_log(filename):
    with open('../data/' + filename) as f, open('../data/' + 'error.csv','w') as csvf:
        log = f.read()
        regexp = r'\[([A-Za-z]{3} [A-Za-z]{3} \d{2} \d{2}:\d{2}:\d{2}).*(\d{4})\] \[(.*?)\] \[(.*?)\] \[client(.*?)\] [A-Z]{3} [A-Za-z]{6}: (.*?)\bin\b(.*(/~ssunar).*)'
        lists = re.findall(regexp,log)
        #creating csv object file
        writer = csv.writer(csvf)
...
if __name__ == '__main__':
    dict_of_pages = {
        'index': '/~arathi',
        'registration': '/~arathi/reg',
        'login': '/~ssunar/login/login',
        'maintenance': '/~ssunar/maintenance/maintenance-page.php',
        'forms': '/~arathi/forms',
        'about_us': '/~arathi/about'
        }
    day='11' #Nov 11 as last thrusday was most traffic
